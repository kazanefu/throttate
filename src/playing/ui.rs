...
impl Plugin for PlayingUiPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(OnEnter(GameState::Playing), spawn_playing_ui)
            .add_systems(Update, (update_time_ui, update_death_count_ui).run_if(in_state(GameState::Playing).and(in_state(RunningState::Running))));
    }
}
...
fn spawn_playing_ui(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands.entity(canvas).add_children(&[time_ui,death_count_ui]);
}

fn update_time_ui(mut time_ui_query: Query<&mut Text, With<TimeUi>>, stopwatch_query: Query<&StopWatch>) {
    let time = if let Ok(stopwatch)=stopwatch_query.single() {stopwatch.now()}else{warn!("find none or multiple stopwatch in world");return;};
    for mut time_ui_text in &mut time_ui_query {
        **time_ui_text = format!("Time: {:.2}",time);
    }
}

fn update_death_count_ui(mut death_count_ui_query: Query<&mut Text, With<DeathCountUi>>, death_count_query: Query<&DeathCount,(With<Player>,Changed<DeathCount>)>) {
    let death_count = if let Ok(death_count) = death_count_query.single() {death_count.0}else{return;};
