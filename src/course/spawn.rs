        ...
        match course {
            Some((_course_entry, course)) => {
                ...
                for entity in &course.entities {
                    let item_entity =
                        spawn_course_from_entities(&mut commands, entity, font.get()).id();
...
fn spawn_course_from_entities<'a>(
    ...
    match &entity.kind {
        ...
        EntityKind::Checkpoint { priority } => {
            commands.spawn(checkpoint::check_point_bundle(x, y, *priority))
        }
        EntityKind::Breakable { required_speed } => {
            commands.spawn(breakable_box::breakable_box_bundle(x, y, *required_speed))
        }
        EntityKind::Death => commands.spawn(death_box::death_box_bundle(x, y)),
        EntityKind::Turret { interval, rotation } => {
            commands.spawn(turret::turret_bundle(x, y, *interval, *rotation))
        }
